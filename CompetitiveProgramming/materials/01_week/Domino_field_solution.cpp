...
uint64 dp[200][10][1100];

ll solve(int32 i, int32 j, uint64 mask, uint64 nextMask){
    if(j>=N){
        i++;
        j=0;
        mask = nextMask;
    ...
    uint64 currMask = nextMask&((1<<j)-1); // First j-1 bits from the next row
