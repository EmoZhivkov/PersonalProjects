...
public class FuelEngineFactoryWithThreeArg implements EngineFactory {
    ...
    public FuelEngineFactoryWithThreeArg(Set<String> fuelTypes, Set<String> powerTypes,
    ...
    public Engine createEngine(String input) throws EngineNotSupportedException{
        ...
        if (emissionStandarts.contains(temp[2])) {
            emissionStandart = temp[2];
        } else {
            throw new EngineNotSupportedException("The Engine you have inputed is not supported. Enter a new command");
        }

        return new FuelEngine(fuelType, powerType, emissionStandart);
    }
}
